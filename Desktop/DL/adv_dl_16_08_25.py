# -*- coding: utf-8 -*-
"""Adv_DL - 16.08.25.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MglIuhqI8f_Y6AszhrZjeQpATQXAlqmS
...
class FeatureExtractor :
    def __init__(self, tfidf_max_features: int = 100, afinn_lex_path : str = 'AFINN-111.txt', nrc_lex_path : str = 'NRC.txt', subj_lex_path : str = 'MPQA.txt') :
...
# 1) Instantiate (paths to your lexicon files) :

fe = FeatureExtractor(afinn_lex_path = 'AFFIN-11.txt', nrc_lex_path = 'NRC.txt', subj_lex_path = 'MPQA.txt')

# 2) Fit+transform on train :
...
# 4) Now `train_meta` and `test_meta` are ready to concat with other metadata and feed into your model :

# Make sure both DataFrames are aligned on the same index -
...
def train_with_best_params(best_params, num_epochs = 5) :
    ...
    torch.save(model.state_dict(), 'roberta_state_dict.pt')
    print('Saved state_dict to roberta_state_dict.pt')
    torch.save(model, 'roberta_full_model.pt')
    print('Saved full model to roberta_full_model.pt')

    return model

model_best = train_with_best_params(best_params_optuna_manual, num_epochs = 5)
...
# Load WandB run history and plot training loss -

api = wandb.Api()
run = api.run('yardenco5900-tel-aviv-university/roberta_hf_optuna/9lcig234')
history = run.history()
...
# Load pre-trained RoBERTa model for sequence classification. We specify 5 labels for multi-class classification and set the dropout rate -

model = RobertaForSequenceClassification.from_pretrained('roberta-base', num_labels = 5, hidden_dropout_prob = best_params_roberta_hf['dropout'])

# Load corresponding tokenizer -

tokenizer = AutoTokenizer.from_pretrained('roberta-base')

# Create dataset using your custom TweetWithMetaDataset. This dataset likely combines text and meta-features -
...
train_args = TrainingArguments(output_dir = './roberta_hf_best',                                      # Where to save model checkpoints
                               learning_rate = best_params_roberta_hf['lr'],                          # Learning rate from hyperparameter search
                               per_device_train_batch_size = best_params_roberta_hf['batch_size'],    # Batch size
                               num_train_epochs = 5,                                                  # Number of training epochs
                               logging_dir = './logs',                                                # Directory for storing logs
                               logging_steps = 100,                                                   # Log metrics every 100 steps
                               report_to = 'none')                                                    # Disable automatic logging to WandB or TensorBoard


# Initialize Hugging Face Trainer -
trainer = Trainer(model = model, args = train_args, train_dataset = train_set, eval_dataset = val_set, tokenizer = tokenizer, compute_metrics = compute_metrics_eval                   )
...
def evaluate_hf_with_loss_plot(model, dataloader, device, wandb_run_path, class_names = None) :
    ...
    fig, ax = plt.subplots(figsize = (8, 6))
    disp.plot(cmap = 'Blues', values_format = 'd', ax = ax, xticks_rotation = 45)
    plt.title('RoBERTa_HF Confusion Matrix')
    plt.tight_layout()
    plt.show()
    ...
    # Fetch WandB run and plot training loss -
...
cm_hf, metrics_hf = evaluate_hf_with_loss_plot(trainer.model,   test_loader, device,
    wandb_run_path = 'yardenco5900-tel-aviv-university/roberta_hf_optuna/9lcig234', class_names = class_names)
...
def train_distilbert_best(best_params, num_epochs = 5) :
    """
    ...
    torch.save(model.state_dict(), 'distilbert_state_dict.pt')
    print('Saved state_dict to distilbert_state_dict.pt')
    torch.save(model, 'distilbert_full_model.pt')
    print('Saved full model to distilbert_full_model.pt')

    return model, history

# Usage -
model_distilbert_manual, history_distilbert = train_distilbert_best(best_params_distilbert_manual, num_epochs = 5)
...
def train_distilbert_hf_best(best_params, num_epochs = 5) :
    """
    Train a DistilBERT model with meta-features using Hugging Face Trainer.

    Parameters
    ----------
    best_params : dict
        Dictionary containing hyperparameters: 'lr', 'dropout', 'batch_size'.
    num_epochs : int, optional
    ...
    # Initialize the model with meta-features -

    model = TransformerWithMeta(model_name, train_meta.shape[1], best_params['dropout'], num_labels = 5).to('cuda')

    # Hugging Face training arguments -

    training_args = TrainingArguments( output_dir = './results', save_strategy = 'no',
        num_train_epochs = num_epochs, per_device_train_batch_size = best_params['batch_size'],
        per_device_eval_batch_size = best_params['batch_size'], learning_rate = best_params['lr'], logging_strategy = 'epoch', report_to = 'none'          )

    # Initialize Hugging Face trainer with custom MetaTrainer -
...
# Train DistilBERT HF model with best hyperparameters -

model_distilbert_HF, tokenizer_distilbert_HF = train_distilbert_hf_best(best_params_distilbert_HF, num_epochs = 5)
...
def run_pruning_only(model, train_loader, test_loader, class_names, device, amount = 0.3, num_epochs = 1) :
    ...
    torch.save(model.state_dict(), 'roberta_pruned_only.pt')
...
def run_kd_no_meta(train_df, val_df, test_df, class_names, device = None) :
    ...
    torch.save(student_model.state_dict(), 'tinybert_student_state_dict.pt')
    torch.save(student_model, 'tinybert_student_full.pt')
...
state_dict = torch.load('roberta_state_dict.pt', map_location = device)
...
# Set the device explicitly to CPU for quantization
device = torch.device('cpu')

# Perform dynamic quantization on the loaded model. Only quantizes nn.Linear layers to reduce memory and improve inference speed. Uses 8-bit integers (qint8) for weights -

q_model = torch.quantization.quantize_dynamic(
    model_loaded.to('cpu'),     # Move model to CPU for quantization
    {nn.Linear},                # Specify which layer types to quantize
    dtype = torch.qint8         # Data type for quantized weights
)

# Evaluate the quantized model using the same test data loader. This computes standard metrics like accuracy, F1, AUC, and inference time -

metrics = evaluate_model(q_model, test_loader, device, class_names)

# Display the computed metrics for the quantized model -
...
# Set device to CPU for training and evaluation -

device = torch.device('cpu')

# Load the TinyBERT tokenizer from HuggingFace -

student_tokenizer = AutoTokenizer.from_pretrained('prajjwal1/bert-tiny')

# Create PyTorch datasets for training, validation, and testing using the TinyBERT tokenizer -

train_dataset_student = TweetDatasetNoMeta(train_df_split, student_tokenizer)
val_dataset_student   = TweetDatasetNoMeta(val_df_split, student_tokenizer)
test_dataset_student  = TweetDatasetNoMeta(test_df, student_tokenizer)

# Train the TinyBERT student model via knowledge distillation without using metadata. Returns the trained student model and evaluation metrics on the test set -

kd_model, kd_metrics = run_kd_no_meta( train_df_split, val_df_split, test_df, class_names, device = device)
...
state_dict = torch.load('distilbert_state_dict.pt', map_location = device)
...
def quantize_distilbert_with_meta(model_with_meta) :
    ...
    q_base_model = torch.quantization.quantize_dynamic(
        base_model.to('cpu'),     # move to CPU for quantization
...
# Prepare the test dataset with metadata using the same tokenizer -
...
# Apply dynamic quantization to the DistilBERT model with metadata -

q_model_distilbert = quantize_distilbert_with_meta(model_distilbert)

# Evaluate the quantized model on the test set (CPU) -

metrics_distilbert_quant = evaluate_model(q_model_distilbert, test_loader_distilbert, device = torch.device('cpu'), class_names = class_names)

# Print the evaluation metrics for the quantized model -
...
metrics_distilbert_pruned = evaluate_model(pruned_model_distilbert, test_loader_distilbert, device, class_names)
...
torch.save(pruned_model_distilbert.state_dict(), 'distilbert_pruned_state_dict.pt')
torch.save(pruned_model_distilbert, 'distilbert_pruned_full.pt')
...
def run_kd_with_tinybert_as_student(teacher_model, train_df, val_df, test_df, class_names, device = None) :
    ...
    torch.save(student_model.state_dict(), 'tinybert_student_from_distilbert_state_dict.pt')
    torch.save(student_model, 'tinybert_student_from_distilbert_full.pt')
    print('Saved TinyBERT student model (teacher = DistilBERT).')

    return student_model, metrics

# Split the original training set into a smaller training set and a validation set (80/20 split) -

train_df_split, val_df_split = train_test_split(train_df, test_size = 0.2, random_state = 42)

# Run Knowledge Distillation using TinyBERT as the student model. The teacher model is DistilBERT with meta-features (model_distilbert) -

kd_model_tinybert, kd_metrics_tinybert = run_kd_with_tinybert_as_student(teacher_model = model_distilbert, train_df = train_df_split, val_df = val_df_split,
    test_df = test_df, class_names = class_names, device = torch.device('cpu'))

"""# **Summary of Model Compression Experiments**

1. **Baseline Models:**

   * **RoBERTa:** Very high accuracy (F1 ≈ 0.96, AUC ≈ 0.996), large (476 MB), slower inference (\~13.6 sec).
   * **DistilBERT:** Smaller (254 MB), faster (\~6.9 sec), slightly lower accuracy (F1 ≈ 0.835, AUC ≈ 0.97).

2. **Quantization:**

   * Cuts model size roughly in half (DistilBERT: 254 → 133 MB, RoBERTa: 476 → 231 MB).
   * Minor performance drop, inference generally faster on smaller models.
   * **Best for moderate compression with low impact on accuracy.**

3. **Pruning:**

   * Removes \~50% of weights but **model size doesn’t shrink much**.
   * Moderate drop in F1 (DistilBERT: 0.71, RoBERTa: 0.84).
   * Slightly faster inference.
   * **Main benefit is computational efficiency, not storage.**

4. **Knowledge Distillation (TinyBERT student):**

   * Massive size reduction (\~16.7 MB), fastest inference (\~4–4.5 sec).
   * Large performance drop (F1 0.36–0.41).
   * **Extreme compression and speed-up, but accuracy suffers.**

### **Key Takeaways**

* **Trade-offs:** Compression improves size and speed but can reduce accuracy.
* **Best methods for deployment:**

  * **Quantization** for balanced size/performance trade-off.
  * **Pruning + quantization** for faster inference without huge storage changes.
* **Distillation** is aggressive and may need a **larger student or longer training** to retain accuracy.
